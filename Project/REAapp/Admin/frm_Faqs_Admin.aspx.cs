...
public partial class Admin_frm_Faqs_Admin : System.Web.UI.Page
{
    ...
    protected void lnkBtn_EditParent_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (READBEntities cntx = new READBEntities())
            {
                ...
                //Label txtExample = (Label)clickedRow.FindControl("lbl_F_PID");
                //int ID = Convert.ToInt32(txtExample.Text);
                //Response.Redirect("frm_FaqsEdit_Admin.aspx?f_pidFE=" + ID);
            }
    ...
    protected void lnkBtn_DeleteParent_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (READBEntities cntx = new READBEntities())
            {
                ...
                Label txtExample = (Label)clickedRow.FindControl("lbl_F_PID");
                int ID = Convert.ToInt32(txtExample.Text);
                var obj = (from q in cntx.tblFaqs where q.f_id == ID select q).FirstOrDefault();
    ...
    protected void lnkBtn_EditChild_Click(object sender, EventArgs e)
    {
        try
        {
    ...
    protected void lnkBtn_DeleteChild_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                RepeaterItem clickedRow = ((LinkButton)sender).NamingContainer as RepeaterItem;
                Label txtExample = (Label)clickedRow.FindControl("lbl_F_CID");
                int ID = Convert.ToInt32(txtExample.Text);
                var obj = (from q in cntx.tblFaqs where q.f_id == ID select q).FirstOrDefault();
                cntx.DeleteObject(obj);
                cntx.SaveChanges();
